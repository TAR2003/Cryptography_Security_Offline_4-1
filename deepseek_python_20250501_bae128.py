import time
import os
from typing import List

# AES implementation with CBC mode and PKCS#7 padding

# S-box and inverse S-box
s_box = (
    ,
)

inv_s_box = (
    ,
)

# Rcon
r_con = (
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40,
    0x80, 0x1B, 0x36, 0x6C, 0xD8, 0xAB, 0x4D, 0x9A,
    0x2F, 0x5E, 0xBC, 0x63, 0xC6, 0x97, 0x35, 0x6A,
    0xD4, 0xB3, 0x7D, 0xFA, 0xEF, 0xC5, 0x91, 0x39,
)

def sub_bytes(s: List[List[int]]) -> List[List[int]]:
    for i in range(4):
        for j in range(4):
            s[i][j] = s_box[s[i][j]]
    return s

def inv_sub_bytes(s: List[List[int]]) -> List[List[int]]:
    for i in range(4):
        for j in range(4):
            s[i][j] = inv_s_box[s[i][j]]
    return s

def shift_rows(s: List[List[int]]) -> List[List[int]]:
    s[1][0], s[1][1], s[1][2], s[1][3] = s[1][1], s[1][2], s[1][3], s[1][0]
    s[2][0], s[2][1], s[2][2], s[2][3] = s[2][2], s[2][3], s[2][0], s[2][1]
    s[3][0], s[3][1], s[3][2], s[3][3] = s[3][3], s[3][0], s[3][1], s[3][2]
    return s

def inv_shift_rows(s: List[List[int]]) -> List[List[int]]:
    s[1][0], s[1][1], s[1][2], s[1][3] = s[1][3], s[1][0], s[1][1], s[1][2]
    s[2][0], s[2][1], s[2][2], s[2][3] = s[2][2], s[2][3], s[2][0], s[2][1]
    s[3][0], s[3][1], s[3][2], s[3][3] = s[3][1], s[3][2], s[3][3], s[3][0]
    return s

def add_round_key(s: List[List[int]], k: List[List[int]]) -> List[List[int]]:
    for i in range(4):
        for j in range(4):
            s[i][j] ^= k[i][j]
    return s

# Galois Field multiplication
def xtime(a: int) -> int:
    return (((a << 1) ^ 0x1B) & 0xFF) if (a & 0x80) else (a << 1)

def mix_single_column(a: List[int]) -> List[int]:
    t = a[0] ^ a[1] ^ a[2] ^ a[3]
    u = a[0]
    a[0] ^= t ^ xtime(a[0] ^ a[1])
    a[1] ^= t ^ xtime(a[1] ^ a[2])
    a[2] ^= t ^ xtime(a[2] ^ a[3])
    a[3] ^= t ^ xtime(a[3] ^ u)
    return a

def mix_columns(s: List[List[int]]) -> List[List[int]]:
    for i in range(4):
        s[i] = mix_single_column(s[i])
    return s

def inv_mix_columns(s: List[List[int]]) -> List[List[int]]:
    for i in range(4):
        u = xtime(xtime(s[i][0] ^ s[i][2]))
        v = xtime(xtime(s[i][1] ^ s[i][3]))
        s[i][0] ^= u
        s[i][1] ^= v
        s[i][2] ^= u
        s[i][3] ^= v
    return mix_columns(s)

def bytes2matrix(text: bytes) -> List[List[int]]:
    return [list(text[i:i+4]) for i in range(0, len(text), 4)]

def matrix2bytes(matrix: List[List[int]]) -> bytes:
    return bytes(sum(matrix, []))

def xor_bytes(a: bytes, b: bytes) -> bytes:
    return bytes(i^j for i, j in zip(a, b))

def pad(plaintext: bytes) -> bytes:
    padding_len = 16 - (len(plaintext) % 16)
    padding = bytes([padding_len] * padding_len)
    return plaintext + padding

def unpad(plaintext: bytes) -> bytes:
    padding_len = plaintext[-1]
    assert padding_len > 0
    message, padding = plaintext[:-padding_len], plaintext[-padding_len:]
    assert all(p == padding_len for p in padding)
    return message

def split_blocks(message: bytes, block_size: int = 16) -> List[bytes]:
    return [message[i:i+block_size] for i in range(0, len(message), block_size)]

def expand_key(master_key: bytes) -> List[List[List[int]]]:
    key_columns = bytes2matrix(master_key)
    iteration_size = len(master_key) // 4
    
    i = 1
    while len(key_columns) < (10 + 1) * 4:
        word = list(key_columns[-1])
        
        if len(key_columns) % iteration_size == 0:
            word.append(word.pop(0))
            word = [s_box[b] for b in word]
            word[0] ^= r_con[i]
            i += 1
        
        word = xor_bytes(bytes(word), bytes(key_columns[-iteration_size]))
        key_columns.append(word)
    
    return [key_columns[4*i : 4*(i+1)] for i in range(len(key_columns) // 4)]

def encrypt_block(plaintext: bytes, key_matrices: List[List[List[int]]]) -> bytes:
    plain_state = bytes2matrix(plaintext)
    
    plain_state = add_round_key(plain_state, key_matrices[0])
    
    for i in range(1, 10):
        plain_state = sub_bytes(plain_state)
        plain_state = shift_rows(plain_state)
        plain_state = mix_columns(plain_state)
        plain_state = add_round_key(plain_state, key_matrices[i])
    
    plain_state = sub_bytes(plain_state)
    plain_state = shift_rows(plain_state)
    plain_state = add_round_key(plain_state, key_matrices[-1])
    
    return matrix2bytes(plain_state)

def decrypt_block(ciphertext: bytes, key_matrices: List[List[List[int]]]) -> bytes:
    cipher_state = bytes2matrix(ciphertext)
    
    cipher_state = add_round_key(cipher_state, key_matrices[-1])
    cipher_state = inv_shift_rows(cipher_state)
    cipher_state = inv_sub_bytes(cipher_state)
    
    for i in range(9, 0, -1):
        cipher_state = add_round_key(cipher_state, key_matrices[i])
        cipher_state = inv_mix_columns(cipher_state)
        cipher_state = inv_shift_rows(cipher_state)
        cipher_state = inv_sub_bytes(cipher_state)
    
    cipher_state = add_round_key(cipher_state, key_matrices[0])
    
    return matrix2bytes(cipher_state)

def handle_key(key: str) -> bytes:
    # Convert key to bytes and handle keys of different lengths
    key_bytes = key.encode('utf-8')
    if len(key_bytes) == 16:
        return key_bytes
    elif len(key_bytes) < 16:
        # Pad with zeros if key is shorter than 16 bytes
        return key_bytes + bytes(16 - len(key_bytes))
    else:
        # Truncate if key is longer than 16 bytes
        return key_bytes[:16]

def encrypt_cbc(plaintext: str, key: str, iv: bytes = None) -> bytes:
    # Handle key
    key_bytes = handle_key(key)
    
    # Generate random IV if not provided
    if iv is None:
        iv = os.urandom(16)
    
    # Pad the plaintext
    plaintext_bytes = plaintext.encode('utf-8')
    padded_plaintext = pad(plaintext_bytes)
    
    # Split into blocks
    blocks = split_blocks(padded_plaintext)
    
    # Expand key
    key_matrices = expand_key(key_bytes)
    
    # Encrypt each block
    ciphertext_blocks = []
    previous = iv
    for block in blocks:
        block = xor_bytes(block, previous)
        encrypted_block = encrypt_block(block, key_matrices)
        ciphertext_blocks.append(encrypted_block)
        previous = encrypted_block
    
    # Return IV + ciphertext
    return iv + b''.join(ciphertext_blocks)

def decrypt_cbc(ciphertext: bytes, key: str) -> str:
    # Handle key
    key_bytes = handle_key(key)
    
    # Extract IV and ciphertext blocks
    iv = ciphertext[:16]
    ciphertext_blocks = split_blocks(ciphertext[16:])
    
    # Expand key
    key_matrices = expand_key(key_bytes)
    
    # Decrypt each block
    plaintext_blocks = []
    previous = iv
    for block in ciphertext_blocks:
        decrypted_block = decrypt_block(block, key_matrices)
        plaintext_block = xor_bytes(decrypted_block, previous)
        plaintext_blocks.append(plaintext_block)
        previous = block
    
    # Unpad and return
    padded_plaintext = b''.join(plaintext_blocks)
    plaintext = unpad(padded_plaintext)
    return plaintext.decode('utf-8')

def main():
    # Get user input
    key = input("Enter encryption key (16 characters recommended): ")
    plaintext = input("Enter plaintext to encrypt: ")
    
    # Time encryption
    start_time = time.time()
    ciphertext = encrypt_cbc(plaintext, key)
    encryption_time = time.time() - start_time
    
    # Time decryption
    start_time = time.time()
    decrypted_text = decrypt_cbc(ciphertext, key)
    decryption_time = time.time() - start_time
    
    # Display results
    print("\nResults:")
    print(f"Original text: {plaintext}")
    print(f"Encrypted (hex): {ciphertext.hex()}")
    print(f"Decrypted text: {decrypted_text}")
    print(f"\nEncryption time: {encryption_time:.6f} seconds")
    print(f"Decryption time: {decryption_time:.6f} seconds")
    
    # Verify correctness
    if plaintext == decrypted_text:
        print("\nVerification: Success! Decrypted text matches original.")
    else:
        print("\nVerification: Failed! Decrypted text does not match original.")

if __name__ == "__main__":
    main()